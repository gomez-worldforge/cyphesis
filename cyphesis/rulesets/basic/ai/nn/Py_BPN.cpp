...
static void BPN_dealloc(PyBPN *self)
{
    self->net.~NN();
    self->ob_type->tp_free(self);
}

static int BPN_init(PyBPN * self, PyObject * args, PyObject * kwds)
{
    if (!PyList_Check(args)) {
       PyErr_SetString(PyExc_TypeError, "BPN() arguments should a list");
       return -1;
    }
    int ts = PyTuple_Size(args);
    if (ts >= 4 || ts != 3) { 
    	PyErr_SetString(PyExc_TypeError, "BPN must take list of 3 ints");
        return -1;
    }
...
static int BPN_run(PyBPN * self, PyObject * args, PyObject * kwds)
{
    if (!PyList_Check(args)) {
       PyErr_SetString(PyExc_TypeError, "BPN() arguments should a list");
       return -1;
    ...
    int ts = PyTuple_Size(args);
    if (ts > 65536) { 
    	PyErr_SetString(PyExc_TypeError, "BPN must take list of less than 65536 items");
        return -1;
    }
    ...
    for (int i = 0; i < ts; i++) { 
    	...
    	if (!PyList_Check(clist)) {
       		PyErr_SetString(PyExc_TypeError, "BPN() arguments should a list of lists");
       		return -1;
    	}
    	int ts2 = PyTuple_Size(clist);
    	if (ts2 > 65536) { 
    		PyErr_SetString(PyExc_TypeError, "BPN must take list of lists of less than 65536 items");
        	return -1;
    	}
...
static PyObject* BPN_test(PyBPN * self, PyObject * args, PyObject * kwds)
{
    if (!PyList_Check(args)) {
       PyErr_SetString(PyExc_TypeError, "BPN() arguments should a list");
       return NULL;
    ...
    for (int i = 0; i < ts; i++) { 
        ...
        for(int j = 0; j < self->net.getinputsize(); j++) {
        	if (PyFloat_Check(item)) {
    			...
    			PyErr_SetString(PyExc_TypeError, "BPN test must take list of floats");
        		return NULL;
    		}
   	}
    }
    ...
    if (output.getcols() != self->net.getoutputsize()) {
    	PyErr_SetString(PyExc_TypeError, "BPN internal error : outputpattern is different from output layer size");
       return NULL;
...
static PyObject * BPN_new(PyTypeObject * type, PyObject *, PyObject *)
{
    // This looks allot like the default implementation, except we call the
    // in-place constructor.
    PyBPN * self = (PyBPN *)type->tp_alloc(type, 0);
    if (self != NULL) {
...
static PyMethodDef BPN_methods[] = {
    {"run",             (PyCFunction)BPN_run,      METH_O},
    {"test",             (PyCFunction)BPN_test,      METH_O},
    {NULL,              NULL}           /* sentinel */
};

PyTypeObject PyBPN_Type = {
        PyObject_HEAD_INIT(0)
        0,                              // ob_size
        //////"nn.BPN",             // tp_name
        "BPN",             // tp_name
        sizeof(PyBPN),             // tp_basicsize
        0,                              // tp_itemsize
        // methods 
        (destructor)BPN_dealloc,   // tp_dealloc
        0,                              // tp_print
        0,                              // tp_getattr
        0,                              // tp_setattr
        0,      // tp_compare
...
PyBPN * newPyBPN()
{
    return (PyBPN *)PyBPN_Type.tp_new(&PyBPN_Type, 0, 0);
}
